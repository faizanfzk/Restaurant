  ...
  return (   
    <div className="App">
      ...
      <div>
        ...
        <button disabled={rating=="asc"}onClick={()=>setRating("asc")}>Sort By Asc</button>
      </div>
      <div>
        <button disabled={cost=="desc"}  onClick={()=>setCost("desc")}>Cost By Desc</button>
        <button disabled={cost=="asc"}onClick={()=>setCost("asc")}>Cost By Asc</button>
      </div>
      <div>
        <h3>Filter Rating</h3>
     ...
     <button disabled={page==5} onClick={()=>setPage(page+1)}>NEXT</button>
      ...
      <div>
     {data.map(item=><Restauraunt key={item.id} {...item}/>
     )} 
      </div>
    </div>
  );
